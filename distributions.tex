...
\begin{alltt}
\hlstd{rSquare} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{y}\hlstd{,} \hlkwc{yhat}\hlstd{,} \hlkwc{intercept}\hlstd{)} \hlkwa{if} \hlstd{(intercept)} \hlkwd{cor}\hlstd{(y, yhat)}\hlopt{^}\hlnum{2} \hlkwa{else} \hlkwd{sum}\hlstd{(yhat}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{/}\hlkwd{sum}\hlstd{(y}\hlopt{^}\hlnum{2}\hlstd{)}

\hlstd{sigmaSquare} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{residuals}\hlstd{,} \hlkwc{p}\hlstd{)} \hlkwd{sum}\hlstd{(residuals}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{/} \hlstd{(}\hlkwd{length}\hlstd{(residuals)} \hlopt{-} \hlstd{p)}

\hlstd{linearRegression} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{y}\hlstd{,} \hlkwc{x}\hlstd{,} \hlkwc{intercept} \hlstd{= T,} \hlkwc{alpha} \hlstd{=} \hlkwa{NULL}\hlstd{) \{}
    \hlstd{x} \hlkwb{<-} \hlkwd{as.matrix}\hlstd{(x)}  \hlcom{# Ensure matrix operations available.}
    \hlkwa{if} \hlstd{(}\hlopt{!}\hlkwd{is.null}\hlstd{(alpha)) y} \hlkwb{<-} \hlstd{y} \hlopt{-} \hlstd{alpha}
    \hlkwa{if} \hlstd{(intercept)       x} \hlkwb{<-} \hlkwd{cbind}\hlstd{(}\hlnum{1}\hlstd{, x)}
    \hlstd{xpx.inv} \hlkwb{<-} \hlkwd{solve}\hlstd{(}\hlkwd{t}\hlstd{(x)} \hlopt{%*%} \hlstd{x)}
    \hlstd{beta}    \hlkwb{<-} \hlstd{xpx.inv} \hlopt{%*%} \hlkwd{t}\hlstd{(x)} \hlopt{%*%} \hlstd{y}
    \hlstd{yhat}    \hlkwb{<-} \hlstd{x} \hlopt{%*%} \hlstd{beta}
    \hlstd{sigma.square}   \hlkwb{<-} \hlkwd{sigmaSquare}\hlstd{(y}\hlopt{-}\hlstd{yhat,} \hlkwd{ncol}\hlstd{(x))}
    \hlkwd{rownames}\hlstd{(beta)} \hlkwb{<-} \hlkwd{paste0}\hlstd{(}\hlstr{'B'}\hlstd{,} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(beta)} \hlopt{-} \hlstd{intercept)}
    \hlkwd{list}\hlstd{(}\hlstr{'coefficients'} \hlstd{= beta,}
         \hlstr{'covariance'}   \hlstd{= xpx.inv} \hlopt{*} \hlstd{sigma.square,}
         \hlstr{'fitted'}       \hlstd{= yhat,}
         \hlstr{'residuals'}    \hlstd{= y} \hlopt{-} \hlstd{yhat,}
         \hlstr{'rsq'}          \hlstd{=} \hlkwd{rSquare}\hlstd{(y, yhat, intercept))}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
